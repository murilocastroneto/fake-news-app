...
*/
 async function publishVote(publish_vote){
     ...
     const factory = getFactory();
   	 ...
   	 if (voter.banned){ //se o peer foi banido, barrar o fluxo, solução temporária até aprender a usar o permissions
     	throw new Error('This member has been banned!');
     }
     ...
     if(nrOfNewsValidations >= byzantineNrOfMembers){
         ...
         for(var j = 0; j < votesList.length; j++){
           ...
           if(countTrue >= byzantineNrOfMembers){
              result = true;
              updateMembersRating(votesList, result, membersList.length); 
              break;
  ...
  */
async function updateMembersRating(votesList, result, membersListSize){
	var numberOfVoters = voteList.length;
    //se menos peers participaram da votação, melhores eles são avaliados (ou piores se noticia não for aprovada)
  	var updateFactorNumber = (membersListSize - voteList.length) * 0.01;
    const participantRegistry = await getPartcipantRegistry('fake.news.biz.Member');
    for(const vote in votesList){
        if(result == vote.vote){
            vote.voter.rating = vote.voter.rating + updateFactorNumber;
          
            if(vote.event_type == PUBLISH){
            	vote.voter.rating = vote.voter.rating + 0.2; //bonus para quem postou a noticia aprovada
            }
          
            if(vote.voter.rating > 10){ //se nota do peer passar de 10, volta para 10
            	vote.voter.rating = 10;
            }
			...
			if(vote.event_type == PUBLISH){
            	vote.voter.rating = vote.voter.rating - 1; //penalidade para quem postou a noticia não aprovada
            }
          
          	if(vote.voter.rating < 5){ //se nota do peer baixar de 5, ele é banido
            	vote.voter.banned = true;
            }
