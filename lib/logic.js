 ...
 */
  async function vote(voteNews){
    ...
    for(var f = 0; f < votesListPreVote.length; f++){
     	if (voter.member_identification == votesListPreVote[f].voter.member_identification){
    ...
    if(result != null){
      var diff = (membersList.length - nrOfNewsValidations);
      if(diff == 0){
          diff = 1;
      }
      var updateFactorNumber = diff * 0.01;
